...
#include "lib/lz.h"
#include "sdram_lz_t210.inl"
#else
#include "sdram_t210.inl"
#endif
...
static void _sdram_config(const sdram_params_t *params)
{
    ...
    MAKE_EMC_REG(EMC_PMACRO_DLL_CFG_0) = params->emc_dll_cfg0;
    MAKE_EMC_REG(EMC_PMACRO_DLL_CFG_1) = params->emc_dll_cfg1;
...
static const void *sdram_get_params()
{
    ...
    LZ_Uncompress(_dram_cfg_lz_t210, buf, sizeof(_dram_cfg_lz_t210));
    return (const void *)&buf[sizeof(sdram_params_t) * fuse_get_dram_id()];
#else
    return _dram_cfgs_t210[fuse_get_dram_id()];
#endif
}

void sdram_init_t210()
{
    volatile tegra_pmc_t *pmc = pmc_get_regs();

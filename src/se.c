...
void decrypt_data_into_keyslot(unsigned int keyslot_dst, unsigned int keyslot_src, const void *wrapped_key, size_t wrapped_key_size) {
    ...
    se->SE_CONFIG = (DEC_ALG_AES_DEC | DST_KEYTAB);
...
void se_synchronous_exp_mod(unsigned int keyslot, void *dst, size_t dst_size, const void *src, size_t src_size) {
    ...
    se->SE_CONFIG = (ENC_ALG_RSA | DST_RSAREG);
...
void se_aes_ctr_crypt(unsigned int keyslot, void *dst, size_t dst_size, const void *src, size_t src_size, const void *ctr, size_t ctr_size) {
    ...
    se->SE_SPARE = 1;
    se->SE_CONFIG = (ENC_ALG_AES_ENC | DST_MEMORY);
    se->SE_CRYPTO_CONFIG = (keyslot << 24) | (1 << CTR_CNTN_SHIFT) | CORE_SEL_ENCRYPT | SE_CRYPTO_CONFIG_AES_CTR;
    set_se_ctr(ctr);
...
void se_aes_ecb_encrypt_block(unsigned int keyslot, void *dst, size_t dst_size, const void *src, size_t src_size, unsigned int config_high) {
    ...
    se->SE_CONFIG = (ENC_ALG_AES_ENC | DST_MEMORY) | config_high;
    se->SE_CRYPTO_CONFIG = keyslot << 24 | CORE_SEL_ENCRYPT | SE_CRYPTO_CONFIG_AES_ECB;
...
void se_aes_256_ecb_encrypt_block(unsigned int keyslot, void *dst, size_t dst_size, const void *src, size_t src_size) {
    se_aes_ecb_encrypt_block(keyslot, dst, dst_size, src, src_size, ENC_MODE_KEY256 | DEC_MODE_KEY256);
}

void se_aes_ecb_decrypt_block(unsigned int keyslot, void *dst, size_t dst_size, const void *src, size_t src_size) {
    volatile tegra_se_t *se = se_get_regs();
    
    se->SE_CONFIG = (DEC_ALG_AES_DEC | DST_MEMORY);
...
void aes_128_xts_nintendo_crypt_sector(unsigned int keyslot_1, unsigned int keyslot_2, size_t sector, bool encrypt, void *dst, const void *src, size_t size) {
    ...
    if (encrypt) {
        se->SE_CONFIG = (ENC_ALG_AES_ENC | DST_MEMORY);
        se->SE_CRYPTO_CONFIG = keyslot_1 << 24 | CORE_SEL_ENCRYPT;
    } else {
        se->SE_CONFIG = (DEC_ALG_AES_DEC | DST_MEMORY);
...
void se_compute_aes_cmac(unsigned int keyslot, void *cmac, size_t cmac_size, const void *data, size_t data_size, unsigned int config_high) {
    ...
    se->SE_CONFIG = (ENC_ALG_AES_ENC | DST_HASHREG) | (config_high << 16);
    se->SE_CRYPTO_CONFIG = (keyslot << 24) | (SE_CRYPTO_CONFIG_AES_CBC_ENCRYPT | HASH_ENB_ENABLE);
    ...
    if (num_blocks > 1) {
        ...
        se->SE_CRYPTO_CONFIG |= IV_SELECT_UPDATED;
    }
...
void se_compute_aes_128_cmac(unsigned int keyslot, void *cmac, size_t cmac_size, const void *data, size_t data_size) {
    se_compute_aes_cmac(keyslot, cmac, cmac_size, data, data_size, ENC_MODE_KEY128 | DEC_MODE_KEY128);
}
void se_compute_aes_256_cmac(unsigned int keyslot, void *cmac, size_t cmac_size, const void *data, size_t data_size) {
    se_compute_aes_cmac(keyslot, cmac, cmac_size, data, data_size, ENC_MODE_KEY256 | DEC_MODE_KEY256);
}
...
void se_aes_cbc_encrypt(unsigned int keyslot, void *dst, size_t dst_size, const void *src, size_t src_size, const void *iv) {
    ...
    se->SE_CONFIG = (ENC_ALG_AES_ENC | DST_MEMORY);
    se->SE_CRYPTO_CONFIG = (keyslot << KEY_INDEX_SHIFT) | SE_CRYPTO_CONFIG_AES_CBC_ENCRYPT;
...
void se_aes_cbc_decrypt(unsigned int keyslot, void *dst, size_t dst_size, const void *src, size_t src_size, const void *iv) {
    ...
    se->SE_CONFIG = (DEC_ALG_AES_DEC | DST_MEMORY);
    se->SE_CRYPTO_CONFIG = (keyslot << KEY_INDEX_SHIFT) | SE_CRYPTO_CONFIG_AES_CBC_DECRYPT;
    set_aes_keyslot_iv(keyslot, iv, 0x10);
    se->SE_CRYPTO_LAST_BLOCK = (src_size >> 4) - 1;
    trigger_se_blocking_op(OP_START, dst, dst_size, src, src_size);
}
...
void se_aes_256_cbc_encrypt(unsigned int keyslot, void *dst, size_t dst_size, const void *src, size_t src_size, const void *iv) {
    ...
    se->SE_CONFIG = (ENC_ALG_AES_ENC | DST_MEMORY) | ENC_MODE_KEY256 | DEC_MODE_KEY256;
    se->SE_CRYPTO_CONFIG = (keyslot << KEY_INDEX_SHIFT) | SE_CRYPTO_CONFIG_AES_CBC_ENCRYPT;
    set_aes_keyslot_iv(keyslot, iv, 0x10);
    se->SE_CRYPTO_LAST_BLOCK = (src_size >> 4) - 1;
    trigger_se_blocking_op(OP_START, dst, dst_size, src, src_size);
...
void se_calculate_sha256(void *dst, const void *src, size_t src_size) {
    ...
    se->SE_CONFIG = (ENC_MODE_SHA256 | ENC_ALG_SHA | DST_HASHREG);
    se->SE_SHA_CONFIG = 1;
...
void se_initialize_rng(unsigned int keyslot) {
    ...
    se->SE_CONFIG = (ENC_ALG_RNG | DST_MEMORY);
    se->SE_CRYPTO_CONFIG = (keyslot << 24) | CORE_SEL_ENCRYPT | INPUT_SEL_RANDOM;
    se->SE_RNG_CONFIG = 5;
    se->SE_CRYPTO_LAST_BLOCK = 0;
